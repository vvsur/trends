...
StrategicInitiative хранит стартовые стратегические инициативы из PDF seed, чтобы они были доступны в UI до появления полного innovation workflow. Записи имеют стабильный `seed_key`, связаны с Department и сохраняют SourceTrace JSON без перенумерации PDF.

| Поле | Тип | Обязательное |
| --- | --- | --- |
| id | UUID/string | да |
| seed_key | string unique | да |
| title | string | да |
| department_id | ref | да |
| created_quarter | string | да |
| owner_name | string | да |
| year | string | да |
| comment | text | да |
| source_trace | json | да |
| version | int | да |
| created_at / updated_at | timestamp | да |

Seed publication writes `AuditEvent` records with `event_type=seed`, `source=seed` and `entity_type=strategic_initiative`.
