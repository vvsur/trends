...
| TR-022 | Импортировать стартовый реестр из материалов | Done | P0 | Product/Data/Backend/Frontend | Codex | 2026-06-04 | Done: `seed:publish` publishes 10 PDF technology trends and 7 strategic initiatives into SQLite; source numbers preserve missing trend `9`; `/trends` and `/innovations` browser checks passed; seed audit events verified |
...
| TR-322 | Заполнить MVP-разделы seed-backed данными | Done | P0 | Product/UX/Data | Codex | 2026-06-04 | Done: `/scoring`, `/pilots`, `/kpi`, `/cases`, `/my-trends` заменены на витрины из PDF seed API; score/pilot/effect facts не выдумываются; typecheck/build/lint/raw-color scan/browser check passed |
| TR-323 | Добавить графическое представление seed-backed данных | Done | P0 | Product/UX/Data | Codex | 2026-06-04 | Done: добавлены bar/funnel/timeline графики для `/scoring`, `/pilots`, `/kpi`, `/cases`, `/my-trends`; данные берутся из seed API без новых вымышленных фактов; typecheck/build/lint/raw-color scan/browser check passed |
| TR-324 | Улучшить отображение Source traceability | Done | P0 | Frontend/Data/UX | Codex | 2026-06-04 | Done: сырой JSON в карточке тренда заменен на читаемый traceability summary с source number, review status, dataset, paths и коротким hash; typecheck/build/lint/raw-color scan/browser check passed |
